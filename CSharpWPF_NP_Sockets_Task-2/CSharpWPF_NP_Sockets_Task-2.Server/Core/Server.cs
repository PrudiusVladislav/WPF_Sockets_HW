...
namespace CSharpWPF_NP_Sockets_Task_2.Server.Core;
    ...
    private void Handle()
    {
        ...
        try
        {
            ...
            var result = receivedMessage.ToLower() switch
            {
                ...
                _ => "The request option was not set correctly"
            };

            var sendBytes = Encoding.ASCII.GetBytes(result);
            server.Send(sendBytes);
            var sentMessageWithInfo = $"At {DateTime.UtcNow.ToString("HH:mm")} a message was sent: {result}";
